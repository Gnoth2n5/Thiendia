...
@section('title', 'Chi tiết lăng mộ ' . $grave->grave_number . ' - Quản lý Nghĩa Địa')
    ...
    <div class="space-y-6">
        ...
        <div class="card bg-base-100 shadow-lg border border-base-300">
            <div class="card-body">
                ...
                <a href="{{ route('search', ['cemetery_id' => $grave->cemetery_id]) }}" class="btn btn-outline btn-sm gap-2 w-full">
                        ...
                        @foreach($grave->contact_info as $key => $value)
                            @if($value)
                                <div class="flex items-start gap-2">
                                    @if($key === 'phone')
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="h-5 w-5 text-primary shrink-0">
                                    ...
                                    @endif
                                    <div class="flex-1">
                                        <p class="text-xs text-base-content/60">{{ ucfirst($key) }}</p>
