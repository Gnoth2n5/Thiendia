...
@section('title', $article->title . ' - Quản lý Nghĩa Địa')
...
<div class="mb-8">
    ...
    @if($article->excerpt)
        <p class="text-xl text-base-content/70 leading-relaxed mb-6">{{ $article->excerpt }}</p>
    @endif
                ...
                <div class="flex items-center gap-3">
                    <div class="avatar placeholder">
                        <div class="bg-primary text-primary-content rounded-full w-12">
                            <span class="text-lg font-bold">{{ substr($article->author->name, 0, 1) }}</span>
                        </div>
                    </div>
                    <div>
                        <div class="font-bold">{{ $article->author->name }}</div>
                        <div class="text-sm text-base-content/60">{{ $article->author->email }}</div>
                    </div>
                </div>
            </div>
        </div>
                            ...
                            <div class="flex gap-3">
                                <div class="flex-shrink-0">
                                    @if($relatedArticle->featured_image)
                                        <img src="{{ $relatedArticle->featured_image }}" alt="{{ $relatedArticle->title }}" class="w-16 h-16 object-cover rounded-lg">
                                    @else
                                ...
                                <div class="flex-1 min-w-0">
                                    <a href="{{ route('articles.show', $relatedArticle->slug) }}" class="block">
                                        <h4 class="font-semibold text-sm leading-tight hover:text-primary transition-colors line-clamp-2">
...
<div class="fixed bottom-8 right-8">
    <button onclick="window.scrollTo({top: 0, behavior: 'smooth'})" class="btn btn-primary btn-circle shadow-lg">
