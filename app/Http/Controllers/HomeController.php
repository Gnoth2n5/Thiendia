...
class HomeController extends Controller
{
    public function index()
    {
        ...
        // Lấy banner images
        $bannerImages = [];
        $bannerSetting = Setting::where('key', 'banner')->where('status', true)->first();
        if ($bannerSetting && ! empty($bannerSetting->value)) {
            $images = json_decode($bannerSetting->value, true);
            if (is_array($images) && ! empty($images)) {
                // Convert paths to full URLs
                $bannerImages = array_map(function ($image) {
                    return Storage::url($image);
                }, $images);
            }
        }

        return view('home', compact('cemeteries', 'totalGraves', 'bannerImages'));
    }
